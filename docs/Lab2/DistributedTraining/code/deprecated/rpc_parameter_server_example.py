...
def run_parameter_server(rank, world_size):
    ...
    print("[ Info ] RPC initialized! Running parameter server...")
    rpc.shutdown()
...
def run_training_loop(rank, num_gpus, train_loader, test_loader):
    ...
    for i, (data, target) in enumerate(train_loader):
        with dist_autograd.context() as cid:
            ...
            if i % 5 == 0:
                print(f"[ Info ] Rank {rank} training batch {i} loss {loss.item()}")
...
if __name__ == '__main__':
    ...
    assert args.rank is not None, "[ Error ] must provide rank argument."
    ...
    if args.rank == 0:
        ...
        run_worker(args.rank, world_size, args.num_gpus, train_loader, test_loader)
